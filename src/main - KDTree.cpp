...
namespace SVO
{
	...
	void TraverseOctree(const Octree& octree, size_t nodeIndex, int currentDepth, const std::function<void(const SVONode&, int)>& operation)
	{
	...
	void TraverseOctree(const Octree& octree, const std::function<void(const SVONode&, int)>& operation)
	{
	...
	void ExtractTrianglesFromOctree(const Octree& octree, std::vector<Triangle>& triangles)
	{
		TraverseOctree(octree, [&triangles](const SVONode& node, int depth)
			{
...
//		//auto te = Time::Now();
//		//SVO::IntegratePointCloud(octree, inputPoints.data(), inputPoints.size(), 500.0f, 12);
//		//Time::End(te, "Integrate PointCloud Patches");
//		//
//		//Time::End(t1, "Integrate PointCloud");
